...
# The Initial Condition is a Gaussian IC
...
data = sio.loadmat('../../Data/Generated_Data/Burgers/FC_gaussian_data.mat', struct_as_record=False, squeeze_me=True)
...
def run_network(size):
  ...
  n = size + 1 # grid size for x and t 
...
if __name__ == '__main__':
    # Size 8000 Training:
    U_8000, error8000 = run_network(8000)

    # Size 4000 Training:
    U_4000, error4000 = run_network(4000)

    # Size 2000 Training:
    U_2000, error2000 = run_network(2000)
    ...
    t_test = np.linspace(0, 6, 201)

    X_test, T_test = np.meshgrid(x_test, t_test)

    # Predicted solution for 8000
    ax = fig.add_subplot(2, 2, 2, projection='3d')
    ax.plot_surface(X_test, T_test, U_8000, cmap='plasma')
    ax.set_xlabel('x', fontsize=12)
    ax.set_ylabel('t', fontsize=12)
    ax.set_zlabel('u(x,t)', fontsize=12)
    ax.set_zticks(np.linspace(0, 1, 6))
    ax.set_title("Predicted Solution For Grid Size: 8000\nError: " + str(np.round(error8000, 4)) + "\n", fontsize=14)


    # Predicted solution for 4000
    ax = fig.add_subplot(2, 2, 3, projection='3d')
    ax.plot_surface(X_test, T_test, U_4000, cmap='plasma')
    ax.set_xlabel('x',fontsize=12)
    ax.set_ylabel('t',fontsize=12)
    ax.set_zlabel('u(x,t)',fontsize=12)
    ax.set_zticks(np.linspace(0, 1, 6))
    ax.set_title("Predicted Solution For Grid Size: 4000\nError: " + str(np.round(error4000, 4)) + "\n", fontsize=14)


    # Predicted solution for 2000
    ax = fig.add_subplot(2, 2, 4, projection='3d')
    ax.plot_surface(X_test, T_test, U_2000, cmap='plasma')
    ax.set_xlabel('x',fontsize=12)
    ax.set_ylabel('t',fontsize=12)
    ax.set_zlabel('u(x,t)',fontsize=12)
    ax.set_zticks(np.linspace(0, 1, 6))
    ax.set_title("Predicted Solution For Grid Size: 2000\nError: " + str(np.round(error2000, 4)) + "\n", fontsize=14)
